        ...
        case MAVLINK_MSG_ID_GPS_RAW_INT:
          {
            ...
            float latitude_degree = gps_raw_int.lat / 1e7;
            float longitude_degree = gps_raw_int.lon / 1e7;
            data_array[9] = String(latitude_degree);
            data_array[10] = String(longitude_degree);
            if (!ready_to_log) {
              ready_to_log = true;
            } else {
...
void readPingData() {
  ...
  if (ping.update()) {
    ...
    // Store the data in the buffer at the current index
    data_buffer[buffer_index] = mm_ft_conversion;
    data_array[2] = String(data_buffer[buffer_index]);
